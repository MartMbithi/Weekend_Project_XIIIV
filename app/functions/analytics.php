...
if ($login_rank == 'Admin') {
    ...
    /* Service Providers */
    $query = "SELECT COUNT(*) FROM ploughing_service_providers";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
    $stmt->bind_result($providers);
    $stmt->fetch();
    $stmt->close();

    /* Requests */
    $query = "SELECT COUNT(*) FROM ploughing_service_request";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
    ...
    $stmt->fetch();
    $stmt->close();
} else if ($login_rank == 'Farmer') {
    /* Requests */
    $query = "SELECT COUNT(*) FROM ploughing_service_request psr
    INNER JOIN farmers f ON f.farmer_id = psr.request_famer_id
    WHERE f.farmer_login_id = '{$login_id}'";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
    $stmt->bind_result($requests);
    $stmt->fetch();
    ...
    WHERE psp.service_provider_login_id = '{$login_id}'";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
    $stmt->bind_result($requests);
    $stmt->fetch();
    ...
    /* Payments */
    $query = "SELECT SUM(payment_amount) FROM payments p
    INNER JOIN ploughing_service_request psr ON p.payment_request_id = psr.request_id
    INNER JOIN ploughing_service_providers psp ON psp.service_provider_id = psr.request_service_provider_id
    WHERE psp.service_provider_login_id = '{$login_id}'";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
